...
PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_0 = r"stage_0/densenet_no_layer_no_tensorrt_0.pth"

# Model without fusion and with tensorrt
PATH_RESNET_NO_LAYER_TENSORRT_ONNX_0 = r"stage_0/densenet_no_layer_tensorrt_0.onnx"
PATH_RESNET_NO_LAYER_TENSORRT_ENGINE_0 = r"stage_0/densenet_no_layer_tensorrt_0.trt"

# Model with Fusion and witout Tensorrt 
PATH_RESNET_LAYER_NO_TENSORRT_0 = r"stage_0/densenet_layer_no_tensorrt_0.pth"
...
# Model without layer fuion without tensorrt 
PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_1 = r"stage_1/densenet_no_layer_no_tensorrt_1.pth"

# Model without fusion and with tensorrt
PATH_RESNET_NO_LAYER_TENSORRT_ONNX_1 = r"stage_1/densenet_no_layer_tensorrt_1.onnx"    
PATH_RESNET_NO_LAYER_TENSORRT_ENGINE_1 = r"stage_1/densenet_no_layer_tensorrt_1.trt"

# Model with Fusion and witout Tensorrt 
PATH_RESNET_LAYER_NO_TENSORRT_MODEL_1 = r"stage_1/densenet_layer_no_tensorrt_1.pth"

# Model with Layer and with TensorRT
PATH_RESNET_LAYER_TENSORRT_ONNX_1 = r"stage_1/densenet_layer_tensorrt_1.onnx"
PATH_RESNET_LAYER_TENSORRT_ENGINE_1 = r"stage_1/densenet_layer_tensorrt_1.trt"
...
# Model without layer fuion without tensorrt 
PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_2 = r"stage_2/densenet_no_layer_no_tensorrt_2.pth"

# Model without fusion and with tensorrt
PATH_RESNET_NO_LAYER_TENSORRT_ONNX_2 = r"stage_2/densenet_no_layer_tensorrt_2.onnx"    
PATH_RESNET_NO_LAYER_TENSORRT_ENGINE_2 = r"stage_2/densenet_no_layer_tensorrt_2.trt"

# Model with Fusion and witout Tensorrt 
PATH_RESNET_LAYER_NO_TENSORRT_MODEL_2 = r"stage_2/densenet_layer_no_tensorrt_2.pth"

# Model with Layer and with TensorRT
PATH_RESNET_LAYER_TENSORRT_ONNX_2 = r"stage_2/densenet_layer_tensorrt_2.onnx"
PATH_RESNET_LAYER_TENSORRT_ENGINE_2 = r"stage_2/densenet_layer_tensorrt_2.trt"
...
stage0 = get_first_stage_densenet(densenet)
stage1 = get_second_stage_densenet(densenet)
stage2 = get_third_stage_densenet(densenet)
...
def save_model_witout_fusion_witout_tensorrt(stage):
    ...
    model_in_stage = None
    if(stage == 0):
        model_in_stage = get_first_stage_densenet(densenet)
        torch.save(model_in_stage.state_dict(), PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_0)
    elif(stage == 1):
        model_in_stage = get_second_stage_densenet(densenet)
        torch.save(model_in_stage.state_dict(), PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_1)
    else:
        model_in_stage = get_third_stage_densenet(densenet)
        torch.save(model_in_stage.state_dict(), PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_2)
...
def save_model_without_fusion_with_tensorrt(stage):
    ...
    if(stage == 0):
        print("\n--------- STAGE 0 -------\n")
        if(not(os.path.exists(PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_0))):
    ...
    if(stage == 1):
        print("\n--------- STAGE 1 -------\n")
        if(not(os.path.exists(PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_1))):
            # Model is not saved, so save it
            save_model_witout_fusion_witout_tensorrt(stage = 1)
        
        create_onnx_and_engine_for_stage1(PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_1, fused=False)

    if(stage == 2):
        print("\n--------- STAGE 2 -------\n")
        if(not(os.path.exists(PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_2))):
            # Model is not saved, so save it
            save_model_witout_fusion_witout_tensorrt(stage = 2)
        
        create_onnx_and_engine_for_stage2(PATH_RESNET_NO_LAYER_NO_TENSORRT_MODEL_2, fused=False)
...
def save_model_with_fusion_without_tensorrt(stage):
    ...
    if(stage == 0):
        print("\n---------STAGE 0-------\n")
        stage_0 = get_first_stage_densenet(densenet)
        fused_model = fuse_resnet(stage_0)
        torch.save(fused_model.state_dict(), PATH_RESNET_LAYER_NO_TENSORRT_0)
    elif stage == 1:
        print("\n---------STAGE 1-------\n")
        stage_1 = get_second_stage_densenet(densenet)
        fused_model = fuse_resnet(stage_1)
        torch.save(fused_model.state_dict(), PATH_RESNET_LAYER_NO_TENSORRT_MODEL_1)

    elif stage == 2:
        print("\n---------STAGE 2-------\n")
        stage_2 = get_third_stage_densenet(densenet)
        fused_model = fuse_resnet(stage_2)
        torch.save(fused_model.state_dict(), PATH_RESNET_LAYER_NO_TENSORRT_MODEL_2)

def save_model_with_fusion_with_tensorrt(stage):
    # First see if model is saved
    if(stage == 0):
        print("\n--------- STAGE 0 -------\n")
        if(not(os.path.exists(PATH_RESNET_LAYER_NO_TENSORRT_0))):
            # Model is not saved, so save it
    ...
    if(stage == 1):
        print("\n--------- STAGE 1 -------\n")
        if(not(os.path.exists(PATH_RESNET_LAYER_NO_TENSORRT_MODEL_1))):
            # Model is not saved, so save it
            save_model_with_fusion_without_tensorrt(stage = 1)
        
        create_onnx_and_engine_for_stage1(PATH_RESNET_LAYER_NO_TENSORRT_MODEL_1, fused=True)

    if(stage == 2):
        print("\n--------- STAGE 2 -------\n")
        if(not(os.path.exists(PATH_RESNET_LAYER_NO_TENSORRT_MODEL_2))):
            # Model is not saved, so save it
            save_model_with_fusion_without_tensorrt(stage = 2)
        
        create_onnx_and_engine_for_stage2(PATH_RESNET_LAYER_NO_TENSORRT_MODEL_2, fused=True)



def create_onnx_and_engine_for_stage0(model_path_stage_0, fused = False, batch_size = 16):
    model = torchvision.models.densenet169()
    densenet = model.features

    stage0_resnet = get_first_stage_densenet(densenet)
...
def create_onnx_and_engine_for_stage1(model_path_stage_1, fused = False, batch_size = 16):
    ...
    # Split the model
    stage0_densenet = get_first_stage_densenet(densenet)
    stage1_densenet = get_second_stage_densenet(densenet)
    ...
    # Set the path 
    onnx_file_name = PATH_RESNET_NO_LAYER_TENSORRT_ONNX_1
    engine_file_name = PATH_RESNET_NO_LAYER_TENSORRT_ENGINE_1

    # Change the path if fusion enabled
    if(fused):
        onnx_file_name = PATH_RESNET_LAYER_TENSORRT_ONNX_1
        engine_file_name = PATH_RESNET_LAYER_TENSORRT_ENGINE_1
...
def create_onnx_and_engine_for_stage2(model_path_stage_2, fused = False, batch_size = 16):
    ...
    stage0_densenet = get_first_stage_densenet(densenet)
    stage1_densenet = get_second_stage_densenet(densenet)
    stage_2_densenet = get_third_stage_densenet(densenet)
    ...
    # SEt the path
        # Set the path 
    onnx_file_name = PATH_RESNET_NO_LAYER_TENSORRT_ONNX_2
    engine_file_name = PATH_RESNET_NO_LAYER_TENSORRT_ENGINE_2

    # Change the path if fusion enabled
    if(fused):
        onnx_file_name = PATH_RESNET_LAYER_TENSORRT_ONNX_2
        engine_file_name = PATH_RESNET_LAYER_TENSORRT_ENGINE_2
