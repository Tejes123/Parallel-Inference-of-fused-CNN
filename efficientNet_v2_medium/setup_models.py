...
from model_partition import get_first_stage_efficientNet, get_second_stage_efficientNet, get_third_stage_efficientNet
...
def save_model_witout_fusion_witout_tensorrt(stage):
    ...
    model_in_stage = None
    if(stage == 0):
        model_in_stage = get_first_stage_efficientNet(model)
        torch.save(model_in_stage.state_dict(), PATH_NO_LAYER_NO_TENSORRT_MODEL_0)
    elif(stage == 1):
        model_in_stage = get_second_stage_efficientNet(model)
        torch.save(model_in_stage.state_dict(), PATH_NO_LAYER_NO_TENSORRT_MODEL_1)
    else:
        model_in_stage = get_third_stage_efficientNet(model)
...
def save_model_with_fusion_without_tensorrt(stage):
    ...
    if(stage == 0):
        ...
        print("\n---------STAGE 2-------\n")
        stage_2 = get_third_stage_efficientNet(model)
...
def create_onnx_and_engine_for_stage0(model_path_stage_0, fused = False, batch_size = 16):
    ...
    stage0_resnet = get_first_stage_efficientNet(model)
...
def create_onnx_and_engine_for_stage1(model_path_stage_1, fused = False, batch_size = 16):
    ...
    # Split the model
    stage0_densenet = get_first_stage_efficientNet(model)
    stage1_densenet = get_second_stage_efficientNet(model)
...
def create_onnx_and_engine_for_stage2(model_path_stage_2, fused = False, batch_size = 16):
    model = model = torchvision.models.efficientnet_v2_m()

    # Split the model
    stage0_densenet = get_first_stage_efficientNet(model)
    stage1_densenet = get_second_stage_efficientNet(model)
    stage_2_densenet = get_third_stage_efficientNet(model)
